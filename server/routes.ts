...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin: Promote auto-ingested conflict to curated
  app.post("/api/admin/conflicts/:id/promote", async (req, res) => {
    try {
      ...
      if (!conflict.isAutoIngested) {
        return res.status(400).json({ error: "Conflict is already curated" });
      }

      // Promote to curated status
      const updated = await storage.updateConflict(req.params.id, {
      ...
      res.status(500).json({ error: "Failed to promote conflict" });
    }
  });

  // Admin: Match auto-ingested articles to existing curated conflict
  app.post("/api/admin/conflicts/:id/match", async (req, res) => {
    try {
      const { targetConflictId } = req.body;

      if (!targetConflictId) {
        return res.status(400).json({ error: "Target conflict ID required" });
      }

      const autoConflict = await storage.getConflict(req.params.id);
      const targetConflict = await storage.getConflict(targetConflictId);

      if (!autoConflict || !targetConflict) {
        return res.status(404).json({ error: "Conflict not found" });
      }

      if (!autoConflict.isAutoIngested) {
        return res.status(400).json({ error: "Source conflict is not auto-ingested" });
      }

      if (targetConflict.isAutoIngested) {
        return res.status(400).json({ error: "Target conflict must be curated" });
      }

      // Extract articles from auto-ingested conflict
      const newArticles = autoConflict.mediaLinks.map(link => ({
        url: link.url,
        title: link.title,
        source: new URL(link.url).hostname.replace('www.', '').split('.')[0],
        publishedAt: new Date().toISOString(),
      }));

      // Add to target conflict's recent articles
      const existingArticles = targetConflict.recentArticles || [];
      const updatedArticles = [...existingArticles, ...newArticles];

      // Update target conflict
      const updated = await storage.updateConflict(targetConflictId, {
        recentArticles: updatedArticles,
        recentDataUpdated: new Date(),
      });

      // Delete the auto-ingested conflict
      ...
      res.status(500).json({ error: "Failed to match articles" });
    }
  });
